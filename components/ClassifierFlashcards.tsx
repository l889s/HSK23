...
export function ClassifierFlashcards({
  ...
  function handleRate(rating: Rating) {
    ...
    if (rating === "again") {
      variant = "error";
      if (wasStable) {
        text = `تنسى أحياناً — هذا طبيعي. ستعود ${dayText}`;
      } else if (isNew) {
        text = `بداية صعبة — ستعود ${dayText}`;
      } else {
        text = `لا بأس! ستعود ${dayText} للتركيز`;
      }
    } else if (rating === "hard") {
      variant = "info";
      if (isNew) {
        text = `تحتاج تكرار — ستعود ${dayText}`;
      } else {
        text = `صعبة عليك — ستعود ${dayText}`;
      }
    } else if (rating === "good") {
      variant = "success";
      if (cur.reps >= 2) {
        text = `✨ ثابتة! المراجعة القادمة ${dayText}`;
      } else {
        text = `أحسنت! المراجعة القادمة ${dayText}`;
      }
    } else {
      // easy
      variant = "violet";
      if (cur.reps >= 3) {
        text = `إتقان رائع — لن تراها إلا ${dayText} ⭐`;
      } else {
        text = `سهلة عليك! المراجعة ${dayText} ⭐`;
      }
    }

    toast(text, variant as any);

    // 🆕 سجّل التقييم في توزيع الجلسة
    setRatingCounts((rc) => ({ ...rc, [rating]: rc[rating] + 1 }));

    // 🆕 أطلق Flash بصري سريع، ثم انتقل
  ...
  function advance() {
    ...
    // 🆕 transition: fade out → change card → fade in
  ...
  // شاشة الإنجاز — ملخص مع توزيع التقييمات والمدة
  if (done) {
      ...
      <div className="fixed inset-0 z-[80] flex flex-col items-center justify-center bg-white p-6">
        <div className="w-full max-w-sm">
                ...
                return (
                  ...
                  >
                    ...
                    <div className="relative flex items-center gap-2.5">
                      <span className={cn("text-base font-extrabold", r.color)}>{r.icon}</span>
                      <span className="text-[13px] font-bold text-ink">{r.label}</span>
                    </div>
                    <div className={cn("relative text-base font-extrabold tabular-nums", r.color)}>
                      {count}
                    </div>
                  </div>
                );
            ...
            <div className="mb-5 flex items-center justify-center gap-2 text-[13px]">
              ...
              <span
                className={cn(
                  ...
                  successRate >= 80 ? "text-mint-deep" :
                  successRate >= 60 ? "text-amber-700" : "text-coral"
                )}
              >
                {successRate}%
              </span>
          ...
          <div className="flex flex-col gap-2.5">
            <button
              onClick={restart}
              className="rounded-lg bg-coral py-3 text-[13px] font-bold text-white shadow-coral"
            >
              مراجعة مرة أخرى
            </button>
            <button
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-1 flex-col items-center justify-center px-5 py-6">
        {/* شريط معلومات السياق — يظهر فقط في وضع المراجعة */}
        {mode === "srs" && card && (() => {
        ...
        <div
          className={cn(
            "relative w-full max-w-md flip-scene transition-all duration-200",
            transitioning && "translate-y-2 opacity-0"
          )}
        >
          {/* 🆕 Flash بصري بعد التقييم */}
          {flashRating && (
            <div
              className={cn(
          ...
          >
            ...
            <div className="flip-face flex cursor-pointer flex-col items-center justify-center rounded-3xl border border-[#F0F0F0] bg-white p-6 text-center shadow-card">
              <div
                className="font-cn text-[120px] font-bold leading-none text-coral"
                dir="ltr"
              >
                {card.char}
              </div>
      ...
      <div className="border-t border-[#F0F0F0] p-4">
        {!flipped && (
          <button
            ...
            className="flex w-full items-center justify-center gap-2 rounded-2xl bg-coral py-3.5 text-sm font-bold text-white shadow-coral"
          >
        ...
        {flipped && mode === "normal" && (
          <button
            onClick={handleNext}
            className="flex w-full items-center justify-center gap-2 rounded-2xl bg-ink py-3.5 text-sm font-bold text-white hover:bg-black"
          >
            التالي
          </button>
        )}
        ...
        {flipped && mode === "srs" && (
          <div className="grid grid-cols-4 gap-1.5">
            <button
              ...
              className="flex flex-col items-center gap-1 rounded-2xl bg-coral-soft py-3 font-bold text-coral hover:bg-coral hover:text-white"
              title="نسيتها (1)"
            >
            ...
            <button
              ...
              className="flex flex-col items-center gap-1 rounded-2xl bg-amber-50 py-3 font-bold text-amber-600 hover:bg-amber-500 hover:text-white"
              title="صعبة (2)"
            >
            ...
            <button
              ...
              className="flex flex-col items-center gap-1 rounded-2xl bg-mint-soft py-3 font-bold text-mint hover:bg-mint hover:text-white"
              title="جيدة (3)"
            >
            ...
            <button
              ...
              className="flex flex-col items-center gap-1 rounded-2xl bg-violet-soft py-3 font-bold text-violet hover:bg-violet hover:text-white"
              title="سهلة (4)"
            >
