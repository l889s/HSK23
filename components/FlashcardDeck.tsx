  ...
  return (
    <div className="mx-auto max-w-2xl px-4 sm:px-5">
      ...
      <div className="relative mb-6 rounded-3xl border border-[#F0F0F0] bg-white p-8 shadow-card min-h-[280px] flex flex-col items-center justify-center text-center">
        ...
        <div className="text-base italic text-muted" dir="ltr">
          {card.pinyin}
        </div>

        {reveal && (
          <div className="mt-6 w-full border-t border-[#F0F0F0] pt-5">
            <div className="mb-2 inline-flex rounded-md bg-coral-soft px-2 py-0.5 text-[11px] font-bold text-coral">
              HSK {card.hsk}
            </div>
            <h3 className="mb-2 text-xl font-extrabold text-ink">{card.ar}</h3>
            <p className="text-sm leading-relaxed text-muted">{card.usage}</p>
            {card.examples[0] && (
              <div className="mt-4 rounded-xl bg-coral-soft/40 px-4 py-2.5 text-sm">
                <div dir="ltr" className="font-cn text-base text-ink">
                  {card.examples[0].zh}
                </div>
      ...
      <div className="flex gap-2.5">
        ...
        <button
          onClick={restart}
          className="flex items-center justify-center rounded-2xl bg-[#F7F7F7] px-4 py-3 text-muted hover:bg-[#EEEEEE]"
          aria-label="إعادة"
        >
